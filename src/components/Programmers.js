...
export default function Programmers() {
  ...
  const getNameOfFeatured = (arr) => {
     const name = arr[id - 1].name
     return name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmer.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setId(id = Number(dev.id)) }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          id !== null
            ? `🎉 Let's celebrate ${getNameOfFeatured(listOfAwesome)}! 🥳`
